...
import type { AgentRunner, DeployerPort, GithubPort, Tester, Workspace } from "./ports.ts";
...
export class Orchestrator {
  ...
  async #handleDevTest(taskId: string): Promise<void> {
    ...
    for (;;) {
      ...
      const commit = await workspace.commitAndPush(task, `${task.jiraKey}: ${truncate(run.finalText)}`);
      engStore.setArtifact(
        taskId,
      ...
      if (result.passed) {
        ...
        engStore.setArtifact(taskId, { pr: this.#proposePr(this.#require(taskId), run.finalText, result.summary, commit.filesChanged) }, now());
  ...
  #proposePr(task: EngTask, devSummary: string, testSummary: string, filesChanged: number): PrArtifact {
    const plan = task.artifacts.plan?.editedText ?? task.artifacts.plan?.text ?? "";
    const body = [
      `Implements ${task.jiraKey}: ${task.title}`,
      task.jiraUrl ? `\n${task.jiraUrl}` : "",
      `\n\n## Changes\n${redactSecrets(truncate(devSummary, 500))}`,
      `\n\n## Tests\n${redactSecrets(truncate(testSummary, 300))}`,
      plan ? `\n\n## Plan\n${redactSecrets(truncate(plan, 1000))}` : "",
    ].join("");
    return {
