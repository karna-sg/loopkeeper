...
describe("buildPlanComment", () => {
  ...
  it("includes the plan text when present (prefers editedText)", () => {
    const task = baseTask({
      artifacts: { ...EMPTY_ARTIFACTS, plan: { text: "raw plan", editedText: "annotated plan", sessionId: null, revision: 0, generatedTs: NOW, approvedTs: null, approvedBy: null, summary: null, steps: null, changedFiles: null, newTests: null, riskFlags: null } },
    });
  ...
  it("falls back to plan.text when editedText is null", () => {
    const task = baseTask({
      artifacts: { ...EMPTY_ARTIFACTS, plan: { text: "raw plan", editedText: null, sessionId: null, revision: 0, generatedTs: NOW, approvedTs: null, approvedBy: null, summary: null, steps: null, changedFiles: null, newTests: null, riskFlags: null } },
    });
